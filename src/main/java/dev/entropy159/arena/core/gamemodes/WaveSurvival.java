...
public class WaveSurvival extends CoOpGamemode {
    ...
    public void onLevelTick(ServerLevel level) {
        ...
        if (isInterval) {
            ...
            if (mobCount == 0) {
                ...
                incrementScore();
                Notification.toAll(Component.translatable("message.arena.waves.survived_wave", currentWave).withStyle(ChatFormatting.GREEN));
                currentWave++;
            }
        }
        sendToAll();
    ...
    public @Nullable Component validateMap(ServerLevel level, ArenaMap arenaMap) {
        ...
        if (!arenaMap.getSpawns(level).containsKey(ENEMY_TEAM))
            return Component.translatable("error.arena.no_enemy_spawns");
        return null;
    }
    ...
    public void onClientRender(GuiGraphics graphics, DeltaTracker tracker) {
        ...
        if (isInterval) {
            RenderingUtils.renderText(graphics, Component.translatable("message.arena.waves.interval", cooldownTicks / 20).withStyle(ChatFormatting.YELLOW), ScreenAnchorPoint.TOP_CENTER);
        } else {
            RenderingUtils.renderText(graphics, Component.translatable("message.arena.waves.mob_count", mobCount, getMobCount()).withStyle(ChatFormatting.YELLOW), ScreenAnchorPoint.TOP_CENTER);
        }
    }
