...
public class ItemListCommand {
    ...
    private static int addList(CommandContext<CommandSourceStack> ctx, ItemList.Mode mode, @Nullable ResourceLocation tagLocation) {
        ...
        ctx.getSource().sendFailure(Component.translatable("error.arena.item_list_already_exists", name));
        return 0;
    }
    ...
    private static int removeList(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.itemLists.containsKey(name)) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.removed_item_list", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.item_list_not_found", name));
        return 0;
    }
    ...
    private static int loadList(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.itemLists.containsKey(name)) {
            ...
            if (handler != null) {
                ...
                ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.loaded_item_list", name).withStyle(ChatFormatting.GREEN), true);
                return 1;
            }
            ctx.getSource().sendFailure(Component.translatable("error.arena.no_inventory_at_pos", pos));
            return 0;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.item_list_not_found", name));
        return 0;
    ...
    private static int saveList(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.itemLists.containsKey(name)) {
            ...
            if (handler != null) {
                ...
                ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.saved_item_list", name).withStyle(ChatFormatting.GREEN), true);
                return 1;
            }
            ctx.getSource().sendFailure(Component.translatable("error.arena.no_inventory_at_pos", pos));
            return 0;
    ...
    private static int getListItem(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.itemLists.containsKey(name) && ctx.getSource().getPlayer() != null) {
            ctx.getSource().getPlayer().addItem(data.itemLists.get(name).getItem());
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.gave_item_list", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.item_list_not_found", name));
        return 0;
