...
public class ArenaCommand {
    ...
    private static int start(CommandContext<CommandSourceStack> ctx) {
        ...
        if (error == null) {
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.match_start").withStyle(ChatFormatting.GREEN), true);
            return 1;
        } else {
    ...
    private static int stop(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.match_stop").withStyle(ChatFormatting.RED), true);
        return 1;
    }
    ...
    private static int setLobbyPos(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.set_lobby_pos", pos.toShortString()).withStyle(ChatFormatting.GREEN), true);
        return 1;
    }
    ...
    private static int createMap(CommandContext<CommandSourceStack> ctx) {
        ...
        if (failureMessage == null) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.added_map", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
    ...
    private static int removeMap(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.mapList.removeMap(name)) {
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.removed_map", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.map_not_found", name).withStyle(ChatFormatting.DARK_RED));
        return 0;
    }
    ...
    private static int updateMap(CommandContext<CommandSourceStack> ctx) {
        ...
        if (map != null) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.updated_map", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.map_not_found", name).withStyle(ChatFormatting.DARK_RED));
        return 0;
            ...
            optionalConfig.ifPresentOrElse(config -> {
                try {
                    ...
                    if (isCorrectType(config.getLoadedConfig().config().get(key), valueObj)) {
                        ...
                        ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.updated_map_config", name, key, value).withStyle(ChatFormatting.GREEN), true);
                        returnCode.set(1);
                    } else {
                        ctx.getSource().sendFailure(Component.translatable("error.arena.invalid_config_value", value).withStyle(ChatFormatting.RED));
                    }
                } catch (Exception e) {
                    ctx.getSource().sendFailure(Component.translatable("error.arena.invalid_config_value", value).withStyle(ChatFormatting.RED));
                }
            }, () -> ctx.getSource().sendFailure(Component.translatable("error.arena.no_config", key, modID).withStyle(ChatFormatting.RED)));
    ...
    private static int removeMapOverride(CommandContext<CommandSourceStack> ctx) {
        ...
        if (map != null) {
            ...
            optionalConfig.ifPresentOrElse(config -> {
                ...
                ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.reset_map_config", name, key).withStyle(ChatFormatting.GREEN), true);
            }, () -> ctx.getSource().sendFailure(Component.translatable("error.arena.no_config", key, modID).withStyle(ChatFormatting.RED)));
    ...
    private static int loadMap(CommandContext<CommandSourceStack> ctx) {
        ...
        if (map != null) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.loaded_map", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.map_not_found", name).withStyle(ChatFormatting.DARK_RED));
        return 0;
    ...
    private static int setMapEnabled(CommandContext<CommandSourceStack> ctx, boolean enabled) {
        ...
        if (map != null) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.%s_map".formatted(enabled ? "enabled" : "disabled"), name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.map_not_found", name).withStyle(ChatFormatting.DARK_RED));
        return 0;
    ...
    private static int listMaps(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.mapList.mapListIsEmpty()) {
            ctx.getSource().sendFailure(Component.translatable("error.arena.no_maps"));
            return 0;
        }
    ...
    private static int getFlag(CommandContext<CommandSourceStack> ctx) {
        ...
        if (ctx.getSource().getPlayer() != null) {
            try {
                ...
                ctx.getSource().sendFailure(Component.translatable("error.arena.team_not_found", teamName));
                return 0;
            }
        }
        return 0;
