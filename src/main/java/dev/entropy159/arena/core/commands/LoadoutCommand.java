...
public class LoadoutCommand {
    ...
    private static int addLoadout(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.loadouts.containsKey(name)) {
            ctx.getSource().sendFailure(Component.translatable("error.arena.loadout_already_exists", name));
            return 0;
        }
        if (ctx.getSource().getPlayer() != null) {
            data.loadouts.put(name, new Loadout(ctx.getSource().getPlayer()));
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.added_loadout", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
    ...
    private static int removeLoadout(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.loadouts.containsKey(name)) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.removed_loadout", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.loadout_not_found", name));
        return 0;
    }
    ...
    private static int giveLoadout(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.loadouts.containsKey(name) && ctx.getSource().getPlayer() != null) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.gave_loadout", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.loadout_not_found", name));
        return 0;
    ...
    private static int updateLoadout(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.loadouts.containsKey(name) && ctx.getSource().getPlayer() != null) {
            data.loadouts.put(name, new Loadout(ctx.getSource().getPlayer()));
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.updated_loadout", name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.loadout_not_found", name));
        return 0;
    ...
    private static int setEnabled(CommandContext<CommandSourceStack> ctx, boolean enable) {
        ...
        if (data.loadouts.containsKey(name)) {
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("message.arena.%s_loadout".formatted(enable ? "enabled" : "disabled"), name).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ctx.getSource().sendFailure(Component.translatable("error.arena.loadout_not_found", name));
        return 0;
    ...
    private static int listLoadouts(CommandContext<CommandSourceStack> ctx) {
        ...
        if (data.loadouts.isEmpty()) {
            ctx.getSource().sendFailure(Component.translatable("error.arena.no_loadouts"));
            return 0;
        }
